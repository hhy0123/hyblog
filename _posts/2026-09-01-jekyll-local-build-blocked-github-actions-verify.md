...
직접 만든 Jekyll 테마로 블로그를 운영하면서, 홈 화면 히어로 영역(검색창이 있는 상단 배너)을 다듬는 작업을 하고 있었다. 검색창이 고정 너비(260px)로 배치돼 있다 보니 그 옆으로 빈 공간이 꽤 남았는데, 이 공간을 장식이 아니라 실제로 쓸모 있는 기능으로 채우고 싶었다.
...
- 히어로 여백을 채우는 즐겨찾기 필터 + 카테고리별 색상 배지 기능을 커밋 하나(5개 파일 변경, 289줄 추가)로 배포 완료
- 로컬 검증 과정에서 실제 렌더링 버그(SVG `fill` 미적용)를 push 전에 잡음
- "로컬 빌드가 안 되면 GitHub Actions API로 확인한다"는, 이 PC 환경에서 반복 가능한 검증 절차를 확립함
- 결과적으로 로컬 툴체인 설치 없이도(오히려 설치했다가 다시 지웠다) 프로덕션과 동일한 환경의 빌드 성공 여부를 확인할 수 있었음
