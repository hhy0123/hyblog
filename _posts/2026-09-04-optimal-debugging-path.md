...
부트캠프에서 Claude Team을 제공받는 것은 큰 이점이다. 하지만 **AI에 물어보기 전에 스스로 생각하는 능력**이 없으면 무용지물이다. 

개발자가 "어디가 문제인지 모르겠어"라고 AI에 넘기면, AI도 할 수 있는 게 많지 않다. 진정한 문제 해결은 **증거를 수집하고, 가설을 세우고, 논리적으로 추론하는 과정**에서 나온다.

프로젝트에서 버그가 터졌을 때, 개발자가 제일 먼저 해야 할 일은 "이게 뭘까?"를 추론하는 것이다. 그런데 대부분의 개발자는 직관에만 의존한다. 환경 문제? 코드 문제? 라이브러리? 각각의 가능성을 어떻게 평가할 것인가?

**이 시뮬레이터는 그래서 만들어졌다.** AI를 쓰기 전에 "내가 할 수 있는 최대한의 사고"를 하도록 유도하기 위해. 여러 증거(flags)를 조합해서 각 버그 원인의 가능성 점수를 계산하는 방식으로 말이다. 증거가 추가될수록 점수는 갱신되고, 점수차가 크면 확신도가 올라간다.
